...
from cffibuilder.error import FFIError, CDefError
from cffibuilder.ffiplatform import VerificationError
...
def test_simple():
    ffi = build_ffi(FakeBackend(), cdef="double sin(double x);")
    m = ffi.dlopen(lib_m)
...
def test_pipe():
    ffi = build_ffi(FakeBackend(), cdef="int pipe(int pipefd[2]);")
    C = ffi.dlopen(None)
    func = C.pipe
...
def test_vararg():
    ffi = build_ffi(FakeBackend(), cdef="short foo(int, ...);")
    C = ffi.dlopen(None)
    func = C.foo
...
def test_no_args():
    ffi = build_ffi(FakeBackend(), cdef="""
        int foo(void);
        """)
    C = ffi.dlopen(None)
...
def test_typedef_array_convert_array_to_pointer():
    builder = Builder()
    ffi = FFI(builder._parser, FakeBackend())
    builder.cdef("""
        typedef int (*fn_t)(int[5]);
        """)
    with ffi._lock:
        type = builder._parser.parse_type("fn_t")
...
def test_define_not_supported_for_now():
    builder = Builder()
    e = py.test.raises(CDefError, builder.cdef, '#define FOO "blah"')
    assert str(e.value) == (
...
def test_unnamed_struct():
    builder = Builder()
    builder.cdef("typedef struct { int x; } foo_t;\n"
                 "typedef struct { int y; } *bar_p;\n")
    assert 'typedef foo_t' in builder._parser._declarations
    assert 'typedef bar_p' in builder._parser._declarations
    assert 'anonymous foo_t' in builder._parser._declarations
    type_foo = builder._parser.parse_type("foo_t")
    type_bar = builder._parser.parse_type("bar_p").totype
...
def test_override():
    builder = Builder()
    C = builder.dlopen(None)
    builder.cdef("int foo(void);")
    py.test.raises(FFIError, builder.cdef, "long foo(void);")
    assert C.foo.BType == '<func (), <int>, False>'
    builder.cdef("long foo(void);", override=True)
...
def test_cannot_have_only_variadic_part():
    ...
    builder = Builder()
    e = py.test.raises(CDefError, builder.cdef, "int foo(...);")
    assert str(e.value) == \
           "foo: a function with only '(...)' as argument is not correct C"

def test_parse_error():
    builder = Builder()
    e = py.test.raises(CDefError, builder.cdef, " x y z ")
    assert re.match(r'cannot parse "x y z"\n:\d+:', str(e.value))

def test_cannot_declare_enum_later():
    builder = Builder()
    e = py.test.raises(NotImplementedError, builder.cdef,
...
def test_bool():
    ffi = build_ffi(None, cdef="void f(bool);")
    #
    ffi = build_ffi(None, cdef="typedef _Bool bool; void f(bool);")

def test_win_common_types():
    from cffibuilder.commontypes import COMMON_TYPES, _CACHE
    from cffibuilder.commontypes import win_common_types, resolve_common_type
    #
...
def test_WPARAM_on_windows():
    ...
    ffi = build_ffi(None, cdef="void f(WPARAM);")

def test__is_constant_globalvar():
    from cffibuilder.cparser import Parser, _get_parser
